...
import { api, ApiUser, ApiMessageMaster, ApiDripSequenceCreatePayload } from "@/lib/api";
...
export function CreateDripForm({ currentUser }: CreateDripFormProps) {
    ...
    const [currentStep, setCurrentStep] = useState({ day: "1", time: "09:00", messageId: "" });
    ...
    const handleDragEnd = (result: DropResult) => {
        ...
        const updatedItems = items.map((item, index) => ({ ...item, sequence_order: index }));
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast({ title: "Error", description: "Failed to create drip sequence.", variant: "destructive" });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-6">
            <Card>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 border p-4 rounded-lg">
                        ...
                        <div className="space-y-2"><Label>Day</Label><Input type="number" value={currentStep.day} onChange={e => setCurrentStep({...currentStep, day: e.target.value})} min="1" /></div>
            ...
            <Card>
                ...
                <CardContent>
                    <DragDropContext onDragEnd={handleDragEnd}>
                        <Droppable droppableId="drip-steps">
                            {(provided) => (
                                                ...
                                                <Draggable key={step.localId} draggableId={step.localId} index={index}>
                                                    {(provided) => (
                                                        <TableRow ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                                                            ...
                                                            <TableCell><Button variant="ghost" size="icon" onClick={() => handleRemoveStep(step.localId)}><X className="h-4 w-4" /></Button></TableCell>
                                                        </TableRow>
                                                    )}
                                                </Draggable>
                                            ))}
            ...
            <div className="flex justify-end gap-2">
                ...
                <Button onClick={handleSubmit} disabled={isLoading}>{isLoading ? "Saving..." : "Save Drip Sequence"}</Button>
            </div>
        </div>
    );
}